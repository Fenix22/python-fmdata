...
from fmdata.orm import FMModel
...
class FMClientTestSuite(unittest.TestCase):
    def test_reset_db(self):
        ...
        student_class_layout.find(query=[{"PrimaryKey": "*"}]).raise_exception_if_has_error().found_set.delete_all_records()
        exam_layout.find(query=[{"PrimaryKey": "*"}]).raise_exception_if_has_error().found_set.delete_all_records()
    ...
    def test_mio2(self):
        result_set = Student.objects.order_by("pk").find(pk="*")[1:10][2:3]
